...
@Component({
  selector: 'app-digi-card-sale',
  templateUrl: './digi-card-sale.component.html',
  styleUrls: ['./digi-card-sale.component.scss']
})
export class DigiCardSaleComponent implements OnInit {
  @Input() data: any = {};
  @Input() config: any = {};
  @Input() id: number;
  @Input() router;
  ...
  async loadData(): Promise<void> {
    console.log(this.data, 'load data of for-sale card1');
    await this.getAddress();
    await this.loadOffChainData();
    await this.loadOwner();
    await this.loadSale();
    console.log(this.data, 'load data of for-sale card');
  ...
  async loadSale(): Promise<void> {
    ...
    if (sale !== null && sale.available) {
      ...
      this.data.price = this.math.toHumanValue(sale.fixedPrice, 18);
  ...
  async loadOffChainData() {
    ...
    if (this.data && this.data.auctionOrSaleData.available) {
